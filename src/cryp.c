...
static uint16_t lo_residue[1 << 4] = {
    0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50a5, 0x60c6, 0x70e7, 0x8108,
    0x9129, 0xa14a, 0xb16b, 0xc18c, 0xd1ad, 0xe1ce, 0xf1ef,
};

static uint16_t hi_residue[1 << 4] = {
    0x0000, 0x1231, 0x2462, 0x3653, 0x48c4, 0x5af5, 0x6ca6, 0x7e97, 0x9188,
...
crc16kermitlite(void *isrc, size_t size) {
    ...
    for(size_t i = 0; i < size; i++) {
        res = (res << 8)
            ^ hi_residue[((res >> 8)
            ^ (rev[src[i] & 0x0f] << 4)) >> 4]
            ^ lo_residue[((res >> 8) ^ (rev[src[i] >> 4])) & 0x0f];
    }
    return (bitrev8(res >> 8) << 8) | bitrev8(res & 0xff);
}
